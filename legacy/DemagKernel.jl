"""
    ...
    Most of the code is inspired by Mumax3: https://mumax.github.io/ A. Vansteenkiste at al., "The design and verification of Mumax3", AIP Advances 4 107133 (2014).
...
## Create another type for kernel::CuArray, mesh::CuArray, etc
## struct CudaDemagKernel ?
struct Kernel{T <: AbstractFloat}
    kernelSize::Array{Int64,1}
    kernelPadSize::Array{Int64,1}
    meshSize::Array{Int64,1}
    meshPBC::Array{Int64,1}
    kernel::Array{Array{T,3},2}
end

## Tranform in constructor of type Kernel
function demagKernel(mesh::Mesh, acc::Float64, cacheDir::String)
    ...
    kernel = calculateKernel(mesh, accuracy)
...
function calculateKernel(mesh::Mesh; accuracy=6.0)
    ...
    kernel[X,X] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
    kernel[X,Y] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
    kernel[X,Z] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
    kernel[Y,Y] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
    kernel[Y,Z] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
    kernel[Z,Z] = zeros(size_padded[Y],size_padded[X],size_padded[Z])
	...
	# Start brute integration, 9 nested loops

    for s in 0:2 # source index Ksdxyz --> K[s,d,x,y,z]?
        println("Calculating demagnetizing tensor, line $(s+1).")
        ##### MODIFY FOR 1-BASED INDEXING !!!!!! ####
        u, v, w = s, (s+1)%3, (s+2)%3 # u = direction of source (s), v & w are the orthogonal directions
        ##########
        # Somar 1 quando indexar o array!
        ##########
        #u += 1
        #v += 1
        #w += 1
        ...
        for z in r1[Z]:r2[Z]
            ...
            for y in r1[Y]:r2[Y]
                ...
                for x in r1[X]:r2[X]
                    ...
                    nx = floor(Int, max(cellsize[X]/maxSize, 1.0) + 0.5)
                    ny = floor(Int, max(cellsize[Y]/maxSize, 1.0) + 0.5)
                    nz = floor(Int, max(cellsize[Z]/maxSize, 1.0) + 0.5)
                    ...
                    if (xw == 4 && yw == 2) || (xw == 2 && yw == 1)
                        println(scale)
                    end
                    #### 1 somado na indexing 
                    surface = cellsize[v+1] * cellsize[w+1] # the two directions perpendicular to direction s
                    charge = surface * scale
                    #### 1 somado na indexing 
                    pu1 = cellsize[u+1]/2.0 # positive pole center
                    pu2 = -pu1             # negative pole center

                    # Surface integral over source cell, accumulate in B
                    B = Array{Float64, 1}(undef, 3)
                    #### 1 somado na indexing de um monte de gente, alem dos ranges dos loops for!
                    for i in 0:nv-1
                        pv = -(cellsize[v+1] / 2.0) + cellsize[v+1]/(2*nv) + i*(cellsize[v+1]/nv)
                        pole[v+1] = pv
                        for j in 0:nw-1
                            pw = -(cellsize[w+1] / 2.0) + cellsize[w+1]/(2*nw) + j*(cellsize[w+1]/nw)
                            pole[w+1] = pw
                            ...
                            for α in 0:nx-1
                                ...
                                for β in 0:ny-1
                                    ...
                                    for γ in 0:nz-1
                                        ...
                                        #points++ # A variavel points não está sendo usada para nada!
                                        pole[u+1] = pu1
                    ...
                    for d in s:2 # destination index Ksdxyz --> k[s,d,x,y,z] ?
                        #println("zw: ",zw)
                        #println("yw: ",yw)
                        #println("xw: ",xw)
                        kernel[s+1, d+1][yw+1, xw+1, zw+1] += B[d+1] # += needed in case of PBC
                        ## Original with [zw, yw, xw] indexing order (WHY?) -> Because of 2nd index moves along x direction and 1st index moves along y direction!
                    end
                end
            end
        end
	...
	for z in 0:size_padded[Z]-1
		for y in 0:size_padded[Y]-1
            for x in (size_padded[X]/2 + 1):1.0:size_padded[X]-1 # Nao seria size/2 somente?? Por causa do 1 based?
                x = Int64(x)
                x2 = Int64(size_padded[X] - x)
				kernel[X,X][y+1,x+1,z+1] = kernel[X,X][y+1,x2+1,z+1]
				kernel[X,Y][y+1,x+1,z+1] = -kernel[X,Y][y+1,x2+1,z+1]
				kernel[X,Z][y+1,x+1,z+1] = -kernel[X,Z][y+1,x2+1,z+1]
				kernel[Y,Y][y+1,x+1,z+1] = kernel[Y,Y][y+1,x2+1,z+1]
				kernel[Y,Z][y+1,x+1,z+1] = kernel[Y,Z][y+1,x2+1,z+1]
				kernel[Z,Z][y+1,x+1,z+1] = kernel[Z,Z][y+1,x2+1,z+1]
            end
		end
	end
	...
	for z in 0:size_padded[Z]-1
		for y in (size_padded[Y]/2 + 1):1.0:size_padded[Y]-1
            y2 = Int64(size_padded[Y] - y)
            y = Int64(y)            
			for x in 0:size_padded[X]-1
				kernel[X,X][y+1,x+1,z+1] = kernel[X,X][y2+1,x+1,z+1]
				kernel[X,Y][y+1,x+1,z+1] = -kernel[X,Y][y2+1,x+1,z+1]
				kernel[X,Z][y+1,x+1,z+1] = kernel[X,Z][y2+1,x+1,z+1]
				kernel[Y,Y][y+1,x+1,z+1] = kernel[Y,Y][y2+1,x+1,z+1]
				kernel[Y,Z][y+1,x+1,z+1] = -kernel[Y,Z][y2+1,x+1,z+1]
				kernel[Z,Z][y+1,x+1,z+1] = kernel[Z,Z][y2+1,x+1,z+1]
	...
	for z in (size_padded[Z]/2 + 1):1.0:size_padded[Z]-1
        z2 = Int64(size_padded[Z] - z)
        z = Int64(z)
		for y in 0:size_padded[Y]-1
			for x in 0:size_padded[X]-1
				kernel[X,X][y+1,x+1,z+1] = kernel[X,X][y+1,x+1,z2+1]
				kernel[X,Y][y+1,x+1,z+1] = kernel[X,Y][y+1,x+1,z2+1]
				kernel[X,Z][y+1,x+1,z+1] = -kernel[X,Z][y+1,x+1,z2+1]
				kernel[Y,Y][y+1,x+1,z+1] = kernel[Y,Y][y+1,x+1,z2+1]
				kernel[Y,Z][y+1,x+1,z+1] = -kernel[Y,Z][y+1,x+1,z2+1]
				kernel[Z,Z][y+1,x+1,z+1] = kernel[Z,Z][y+1,x+1,z2+1]
            end
		end
	end
...
function delta(d::Int)
	if d < 0 # Was 0 because of 0-based indexing. CHECK!
		d = -d
    end
	if d > 0 # Was 0 because of 0-based indexing. CHECK!
		d -= 1
...
function zeros(a::Array{Int64,1})
    s = (a[1],a[2],a[3])
    return zeros(s)
end
...
#####################################################################################
## Redundant after I included similar code in the Mesh() creator in MeshGeometry.jl.
## The size_padded is now included in the Mesh type.
#####################################################################################
